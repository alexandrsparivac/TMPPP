...
from ..models.task_prototype import TaskPrototypeRegistry
...
class TaskHandler:
    ...
    async def handle_add_task_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        try:
            ...
            templates = TaskPrototypeRegistry.list_templates()
            keyboard = [
                [InlineKeyboardButton(f"📋 {t.capitalize()}", callback_data=f"template_{t}")
                 for t in templates[i:i+2]]
                for i in range(0, len(templates), 2)
            ]
            keyboard.append([InlineKeyboardButton("✏️ Custom title", callback_data="template_custom")])

            await update.message.reply_text(
                "➕ **Add a new task**\n\nChoose a template or enter a custom title:",
    ...
    async def _create_and_confirm_task(self, update: Update, user, task_title: str) -> None:
        ...
        await update.message.reply_text(
            f"✅ Task added: **{task.title}**\n\n🎯 Customize your task using the buttons below:",
    ...
    async def handle_callback_query(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        try:
            ...
            if callback_data.startswith('template_'):
                template_name = callback_data.split('_', 1)[1]
                user = await self._get_user_from_update(update)
                if not user:
                    return
                if template_name == 'custom':
                    context.user_data['pending_action'] = 'new_task_title'
                    await query.edit_message_text("📝 Enter the task name:")
                else:
                    context.user_data['pending_action'] = f'template_title_{template_name}'
                    await query.edit_message_text(
                        f"📋 Creating **{template_name}** task\n\n📝 Enter the task title:"
                    )
                return
    ...
    async def _process_set_priority(self, query, task: Task, priority_value: str) -> None:
        try:
            priority_map = {
                'urgent': TaskPriority.URGENT, 'high': TaskPriority.HIGH,
    ...
    async def _handle_set_deadline(self, query, task: Task, user: User, context) -> None:
        try:
            context.user_data['pending_action'] = 'set_deadline'
    ...
    async def _handle_add_tags(self, query, task: Task, user: User, context) -> None:
        try:
            context.user_data['pending_action'] = 'add_tags'
            context.user_data['task_id'] = task.id
            await query.edit_message_text(
    ...
    async def _handle_edit_title(self, query, task: Task, user: User, context) -> None:
        try:
            ...
            context.user_data['task_id'] = task.id
            await query.edit_message_text(f"📝 **Edit title for:** {task.title}\n\nSend the new title:")
        except Exception as e:
    ...
    async def _handle_edit_deadline(self, query, task: Task, user: User, context) -> None:
        try:
            context.user_data['pending_action'] = 'edit_deadline'
            context.user_data['task_id'] = task.id
    ...
    async def _handle_edit_tags(self, query, task: Task, user: User, context) -> None:
        try:
            context.user_data['pending_action'] = 'edit_tags'
            context.user_data['task_id'] = task.id
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        try:
            ...
            if pending_action.startswith('template_title_'):
                template_name = pending_action.replace('template_title_', '')
                user_data.pop('pending_action', None)
                
                if template_name == 'meeting':
                    from datetime import timedelta
                    task = await self._get_tasks_use_case._task_repository.create_team_meeting_via_builder(user, message_text, "TBD", datetime.utcnow() + timedelta(days=1))
                    # Actually we should use the use_case directly:
                    task = await self._create_task_use_case.create_team_meeting(user, message_text, "TBD", datetime.utcnow() + timedelta(days=1))
                elif template_name == 'homework':
                    from datetime import timedelta
                    task = await self._create_task_use_case.create_assignment(user, message_text, "General", datetime.utcnow() + timedelta(days=7))
                else:
                    task = TaskPrototypeRegistry.create_from(template_name, user.id, message_text)
                    task_id = await self._task_repository.create(task)
                    task.id = task_id
                
                keyboard = self._create_task_actions_keyboard(task.id)
                await update.message.reply_text(
                    f"✅ Task added from **{template_name}** template: **{task.title}**\n\n"
